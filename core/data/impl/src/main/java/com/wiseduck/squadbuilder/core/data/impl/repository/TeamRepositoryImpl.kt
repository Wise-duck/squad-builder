...
internal class TeamRepositoryImpl @Inject constructor(
    ...
    override suspend fun createTeam(name: String): Result<TeamModel> =
        runCatching {
            val request = TeamCreationRequest(name)
            service.createTeam(request).toModel()
        }

    override suspend fun getTeams(): Result<List<TeamModel>> =
        runCatching {
            service.getTeams().map { it.toModel() }
        }

    override suspend fun deleteTeam(teamId: Int): Result<Unit> =
        runCatching {
