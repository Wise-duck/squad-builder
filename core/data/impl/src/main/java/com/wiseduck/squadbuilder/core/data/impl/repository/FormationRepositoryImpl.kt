...
internal class FormationRepositoryImpl @Inject constructor(
    ...
    override suspend fun getFormationList(teamId: Int): Result<List<FormationListItemModel>> =
        runCatching {
            service.getFormationList(teamId).map { it.toModel() }
        }

    override suspend fun getFormationDetail(formationId: Int): Result<FormationDetailModel> =
        runCatching {
            service.getFormationDetail(formationId).toModel()
        }

    override suspend fun createFormation(request: FormationSaveModel): Result<Unit> =
        runCatching {
    ...
    override suspend fun deleteFormation(formationId: Int): Result<Unit> =
        runCatching {
